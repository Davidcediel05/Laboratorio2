...
def cargar_señal(file_path, num_channels=2, Fs=360):
    Ts = 1 / Fs
...
def metricas_frecuencia(frequencies, spectrum):
...
def plot_correlacion(x1, x2, corr):
    plt.figure(figsize=(10, 4))
...
signal, t, Fs, Ts = cargar_señal(file_path)
statistics = metricas(signal)
frequencies, spectrum = tfourier(signal, Ts)
freqs, psd_ch1, psd_ch2 = _psd(signal, Fs)
mean_freq, median_freq, std_freq = metricas_frecuencia(frequencies, spectrum)


x1 = np.array([1, 0, 0, 0, 9, 7, 1, 3, 6, 4])
h1 = np.array([5, 6, 0, 0, 6, 1, 6])
y1 = convolucion(x1, h1)
plot_convolucion(x1, h1, y1)
x2 = np.array([1, 0, 2, 5, 4, 6, 1, 2, 4, 5])
h2 = np.array([5, 6, 0, 0, 6, 1, 1])
y2 = np.convolve(x2, h2)
plot_convolucion(x2, h2, y2)


x2 = np.cos(2 * np.pi * 100 * np.arange(9) * Ts)
x3 = np.sin(2 * np.pi * 100 * np.arange(9) * Ts)
corr = correlacion(x2, x3)
plot_correlacion(x2, x3, corr)
